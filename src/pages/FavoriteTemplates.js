...
function FavoriteTemplates({ user }) {
    let [templatesList, setTemplatesList] = useState(null);
    
    useEffect(() => {
        let template = []

        user?.userInfo.templateLiked.forEach(id => {
...
export default connect(mapStateToProps, null)(FavoriteTemplates)
